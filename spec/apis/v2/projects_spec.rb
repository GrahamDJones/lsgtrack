...
describe "/api/v2/projects", type: :api do
  ...
  context "projects viewable by this user" do
    ...
    it "json" do
      ...
      projects.any? do |p|
        p["project"]["title"] == @project.name
      end.should be_true
      projects.any? do |p|
        p["project"]["title"] == @project2.name
      end.should be_false
    end

    it "XML" do
      get "#{url}.xml", token: token
      response.body.should eql(Project.for(user).to_xml(options))
      projects = Nokogiri::XML(response.body)
      projects.css("project title").text.should eql(@project.name)
